...
#' or as matrices compressed to a `.qs2` file (`FALSE`)? `TRUE` by default.
#' @param ... additional arguments passed to [terra::writeRaster] or
#' [qs2::qs_save] (if `raster` is `FALSE`).
  ...
  for (att in traits) {
    ...
    if (inherits(x$traits[[att]], "matrix")) {
      if (raster) {
        ...
        full_path <- file.path(
          path,
          paste0(prefix, x$name, "_", att, ".qs2")
        )
        if (overwrite == "skip" && file.exists(full_path)) {
          next
        } else if (overwrite == "error") {
          ...
          qs2::qs_save(x$traits[[att]], full_path, ...)
        } else {
          checkmate::assert_path_for_output(
            full_path,
            overwrite = TRUE
