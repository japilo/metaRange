...
# test saving as .qs2 when raster = FALSE
res_qs <- save_species(
  test_sim$species_01,
  traits = "trait_02",
  prefix = file_prefix,
...
expect_true(
  file.exists(res_qs),
  info = "Saving a matrix trait as .qs2 creates the file."
)
loaded_qs <- qs2::qs_read(res_qs)
expect_equivalent(
  round(loaded_qs),
  round(test_sim$species_01$traits[["trait_02"]]),
  info = "Saved .qs2 file loads and matches the original matrix (rounded)."
)

# cleanup
unlink(c(res_path, res_path3, res_qs))
stopifnot(all(!file.exists(res_path, res_path3, res_qs)))
